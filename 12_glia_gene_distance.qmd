...
subtitle: "Spatial validation of Layers 2 and 3 of the three-layer glial response model"
date: last-modified
---

# Rationale

Notebook 7 established that microglia activate a DAM transcriptional programme near
parenchymal plaques and a distinct cholesterol/lipid programme near CAA. This
constitutes spatial evidence for **Layer 1 (Initiation)** of the proposed three-layer
glial response model.
...
By restricting the analysis to a single cell type at a time we remove the cell-type
composition confound: any residual gene-distance relationship reflects transcriptional
changes within that cell type rather than differential abundance near plaques.
...
## Identify cell-type cluster labels
...
ct_opc   <- all_types[grepl("^opc$|^opcs?$|precursor", all_types, ignore.case = TRUE)]

message("Astrocyte clusters:      ", paste(ct_astro, collapse = ", "))
message("Oligodendrocyte clusters: ", paste(ct_oligo, collapse = ", "))
message("OPC clusters:            ", paste(ct_opc,   collapse = ", "))
```
...
These functions are identical to notebook 7. For each gene we compute a Spearman rank
correlation between per-cell raw expression counts and distance to the nearest plaque
of the specified type. The sign is flipped so that positive `rho_flip` = enriched near
plaques.
...
cor_caa_astro <- compute_gene_distance_correlation(
  slides_astro, dist_astro, distance_col = "dist_caa"
)

cor_paren_astro <- compute_gene_distance_correlation(
  slides_astro, dist_astro, distance_col = "dist_paren"
)
```
...
cor_caa_oligo <- compute_gene_distance_correlation(
  slides_oligo, dist_oligo, distance_col = "dist_caa"
)

cor_paren_oligo <- compute_gene_distance_correlation(
  slides_oligo, dist_oligo, distance_col = "dist_paren"
)
```

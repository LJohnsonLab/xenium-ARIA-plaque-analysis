...
dot_plot_df <- dot_df |> filter(pct >= 1)
adu_range  <- range(dot_plot_df$adu_frac, na.rm = TRUE)
adu_mid    <- 0.5 #mean(adu_range)

ggplot(dot_plot_df, aes(x = banksy_domain, y = annotatedclusters_finer,
                        size = pct, fill = adu_frac)) +
  geom_point(shape = 21, colour = "grey30", stroke = 0.3) +
  scale_size_area(max_size = 10, limits = c(0, 100), name = "% cells") +
  scale_fill_gradient2(low = "#2166AC", mid = "white", high = "#B2182B",
                       midpoint = adu_mid, limits = adu_range,
                       na.value = "grey80", name = "Adu fraction",
                       breaks = c(0.2, adu_mid, round(adu_range[2],1))) +
